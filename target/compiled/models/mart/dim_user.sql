...
--  * email, marketing_channel: Direct passthrough from EMPATHY_DATABASE.postgres_reporting.stg_users
--
-- DERIVED FIELDS:
...
--  * first_event_ts: MIN event timestamp across all user events (aggregate from EMPATHY_DATABASE.postgres_reporting.stg_events)
--  * last_event_ts: MAX event timestamp across all user events (aggregate from EMPATHY_DATABASE.postgres_reporting.stg_events)
--
...
select *
from EMPATHY_DATABASE.postgres_reporting.stg_users