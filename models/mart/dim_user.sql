...
--  * email, marketing_channel: Direct passthrough from {{ ref('stg_users') }}
--
-- DERIVED FIELDS:
...
--  * first_event_ts: MIN event timestamp across all user events (aggregate from {{ ref('stg_events') }})
--  * last_event_ts: MAX event timestamp across all user events (aggregate from {{ ref('stg_events') }})
--
